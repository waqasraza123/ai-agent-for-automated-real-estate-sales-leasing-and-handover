...
export function getCaseManualReplyCopy(locale: SupportedLocale) {
  if (locale === "ar") {
    return {
      ...
      summary: "أرسل الرد البشري عبر واتساب باعتباره قناة العميل الأساسية، مع إبقاء حالة الإرسال والمحادثة والخط الزمني متزامنة.",
      title: "إرسال رد واتساب"
    };
  }
  ...
  return {
    ...
    nextActionDueAt: "Next action due",
    sentByName: "Sent by",
    summary: "Send the human reply through WhatsApp as the primary customer channel while keeping delivery state, conversation history, and follow-up context aligned.",
