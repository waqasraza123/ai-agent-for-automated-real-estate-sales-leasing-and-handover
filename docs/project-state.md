...
- The API now also supports persisted document uploads and downloads through a local storage adapter, with raw file intake, upload metadata persistence, and authenticated download streaming wired into the live case detail contract
...
- The document-analysis pipeline now also records extraction provenance end to end: preview-first parsing is enabled by default, optional Tesseract OCR can be enabled for image uploads, and each upload analysis persists extraction source, extraction status, and extraction-failure detail alongside the existing recommendation and evidence fields
...
- The first provider-ready decision-model upgrade is now live locally, and document intelligence over uploaded evidence now includes persisted extraction provenance and optional OCR, so the next step should be live-model rollout plus broader bilingual and exception scenario coverage rather than more extraction plumbing
